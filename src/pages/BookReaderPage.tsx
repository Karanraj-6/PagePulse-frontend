...
const MOCK_API_RESPONSE = {
  ...
  html: `<h1>Chapter I</h1><p>Tom!</p><p>No answer.</p><p>"Tom!"</p><p>No answer.</p><p>"What's gone with that boy, I wonder? You TOM!"</p><img src="https://www.gutenberg.org/cache/epub/74/images/74-1.jpg" style="max-height: 250px; width:auto; margin: 20px auto; display:block;" /><p>The old lady pulled her spectacles down and looked over them about the room.</p>`
};
...
const BookReaderPage = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setBookMetadata({ title: data.title, author: data.author, id: data.id, cover: data.cover });
      const contentPages = splitHtmlIntoPages(data.html);
      const coverPage = `<div style="height:100%; display:flex; flex-direction:column; justify-content:center; align-items:center; background:#1a1a1a;"><img src="${data.cover}" style="max-height:80%; max-width:90%; box-shadow: 0 10px 30px rgba(0,0,0,0.5);" /><h1 style="color:#d4af37; margin-top:20px; font-size:1.5em; text-align:center;">${data.title}</h1></div>`;
